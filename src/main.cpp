...
bool updateMarketSpotFromApi(std::vector<Option>& options) {
    ...
    int result = std::system("./fetch_data.sh AAPL > /dev/null 2>&1");

    if (result != 0) {
        result = std::system("../fetch_data.sh AAPL > /dev/null 2>&1");
    }
    ...
    if (!readLatestClosePrice("data/AAPL_latest_price.csv", latestSpot) &&
        !readLatestClosePrice("../data/AAPL_latest_price.csv", latestSpot)) {
        return false;
    }
    ...
    for (Option& option : options) {
        if (option.name.find("Apple") != std::string::npos ||
            option.name.find(symbol) != std::string::npos) {
...
int main() {
    ...
    if (marketDataUpdated) {
        std::cout << "Market data: AAPL spot price updated from Alpha Vantage.\n\n";
    } else {
